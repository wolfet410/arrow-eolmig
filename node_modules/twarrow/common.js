...
module.exports.nextFail = function(base, output) {
    ...
    twarrow.Common.log('FAIL:' + output.caller + ',status:' + output.status + ',message:' + s);
...
module.exports.nextSuccess = function(base, output) {
    ...
    twarrow.Common.log('SUCCESS:' + output.caller + ',status:' + output.status + ',message:' + s);
    base.next(null, output.data);
}

module.exports.log = function(msg) {
	// Messages only sent while debugging
    if (twarrow.Constants.DEBUGLOGGING) {
...
module.exports.sendEmail = function(to, subject, body) {
    ...
    if (typeof twarrow.Constants.MAILSERVER === 'undefined' || 
            typeof twarrow.Constants.SMTPPORT === 'undefined') {
        deferred.reject({ success: false, status: 500, caller: 'common.js>sendEmail', 
            data: 'Missing MAILSERVER or SMTPPORT constants in twarrow.Constants' });
    }

    var transporter = Nodemailer.createTransport(SmtpTransport({
            host: twarrow.Constants.MAILSERVER,
            port: twarrow.Constants.SMTPPORT
        }));
        ...
        if (typeof object[key] !== check[key]) {
            if (typeof check[key] === 'object') {
                ...
                if (typeof object[key] === 'undefined') {
                    twarrow.Common.log('common.js>checkObject, mismatch: ' + key 
                        + ' is undefined and should be an object');
         
                    return false;
                }
                var d = twarrow.Common.checkObject(object[key], check[key]);
                if (d === false) {
                    return false;
                }
            } else {
