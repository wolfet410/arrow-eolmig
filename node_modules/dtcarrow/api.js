...
module.exports.testArrowAdmin = function(req) {
    // Testing if this API request is coming from https://arrow-inventory.datatechcafe.com/arrow's Data view
    // These requests include an authorization header, which is a Base 64 encode of Arrow instance's dev apikey.
    if (typeof req.headers.authorization !== 'undefined') {
        var decoded = atob(req.headers.authorization.split(' ')[1]).slice(0, -1);
        if (decoded === req.server.config.apikey) {
            // Base 64 encode and Arrow apikey match, authorize the API to run
            return { success: true, status: 200, caller: 'api.js>testArrowAdmin', data: true };
        } else {
            // No match, something is fishy, so deny the API request
            return { success: false, status: 500, caller: 'api.js>testArrowAdmin', data: false };
        }
    } else {
        // Requests to Arrow do not generally use traditional authorization headers unless they are coming 
        // from the Arrow admin website, so we do not expect to see them and can continue when we don't
        return { success: true, status: 200, caller: 'api.js>testArrowAdmin', data: true };
    }
};
...
module.exports.getPlayerCoachId = function(req, type) {
    // Gets the playerId from the apikey
    var deferred = Q.defer();
    
    if (typeof req.headers['x-api-key'] === 'undefined') {
        deferred.reject({ success: false, status: 404, caller: 'api.js>getPlayerCoachId',
            data: 'Missing x-api-key header (clientApikey)' });
        return deferred.promise;
    }

    if (type.toLowerCase() !== 'player' && type.toLowerCase() !== 'coach' && type.toLowerCase() !== 'unknown') {
        deferred.reject({ success: false, status: 422, caller: 'api.js>getPlayerCoachId',
            data: 'type must be either "player", "coach", or "unknown"' });
        return deferred.promise;   
    }
    ...
    Dtcarrow.Api.getUserDetails(req)
        .then(function(getUserDetailsResult) {
            o.admincoachplayerId = getUserDetailsResult.data.reduceduser.admincoachplayerId;
            return Dtcarrow.User.getType(getUserDetailsResult.data.reduceduser.userId);
        })
        .then(function(getTypeResult) {
            if (type !== 'unknown' && getTypeResult.data.toLowerCase() !== type.toLowerCase()) {
                throw 'Not ' + type.toLowerCase() + ' type';
            } else if (type === 'unknown') {
                deferred.resolve({ success: true, status: 200, caller: '>api.js>getPlayerCoachId', 
                    data: { type: getTypeResult.data.toLowerCase(), id: o.admincoachplayerId } });
            } else {
                // Return just the Id
                deferred.resolve({ success: true, status: 200, caller: '>api.js>getPlayerCoachId', 
                    data: o.admincoachplayerId });
            } 
        })
