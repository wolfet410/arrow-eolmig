...
module.exports.getType = function(userId) {
    // Returns the user type text of a given userId, as a string
    // in either 'Admin', 'Coach', or 'Player'
    var deferred = Q.defer();

    if (typeof userId === 'undefined') {
        deferred.reject({ success: false, status: 404, caller: 'user.js>getType', 
            data: 'Missing userId parameter' });
        return deferred.promise;
    }
    ...
    Model.query({ customSqlQuery: query }, function(err, rs) {
        if (err) {
            deferred.reject({ success: false, status: 500, caller: 'user.js>getType',
                data: err });
            return deferred.promise;
        }
        ...
        for (var i = 0; i < l; i++) {
            if (rs[i].type !== 'N/A') {
                deferred.resolve({ success: true, status: 200, caller: 'user.js>getType',
                    data: rs[i].type.toString() });
                found = true;
                break;
            }
        }
    ...
    Model.query({ apikey: apikey.apikey }, function(err, userresults) {
        ...
        if (userresults[0].userId === 1) {
            deferred.reject({ success: false, status: 422, caller: 'user.js>getDetails',
                data: 'Bad internalapi@datatechcafe.com user returned in results' });
            return deferred.promise;
        }
...
module.exports.readAll = function() {
    ...
    var query = 'SELECT * FROM ViewUserList'
